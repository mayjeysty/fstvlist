<div>
    ...
    <div class="ds-card">
        ...
        <div class="ds-form">
                ...
                <div class="ds-form__radio-group">
                    @foreach(['Tuan' => 'Tuan', 'Nyonya' => 'Nyonya', 'Nona' => 'Nona'] as $val => $label)
                        <label class="ds-form__radio">
                            <input type="radio" name="title" value="{{ $val }}" {{ $loop->first ? 'checked' : '' }}>
            ...
            <div class="ds-form__group">
                ...
                <input type="text" value="{{ old('name', auth()->user()->name) }}" class="ds-form__input" placeholder="Nama sesuai KTP">
            </div>
            <div class="ds-form__group">
                <label class="ds-form__label ds-form__label--required">Alamat Email</label>
                <input type="email" value="{{ old('email', auth()->user()->email) }}" class="ds-form__input" placeholder="kamu@email.com">
            </div>
            <div class="ds-form__group">
                <label class="ds-form__label ds-form__label--required">Nomor WhatsApp</label>
                <div class="ds-input-group">
                    <span class="ds-input-group__prefix">+62</span>
                    <input type="tel" class="ds-input-group__input" placeholder="812-3456-7890">
                </div>
            </div>
            <div class="ds-form__group">
                <label class="ds-form__label ds-form__label--required">NIK</label>
                <input type="text" class="ds-form__input" placeholder="16 digit NIK" maxlength="16">
            </div>
        </div>
    ...
    <div style="display:flex;gap:var(--space-3);margin-bottom:var(--space-8);">
        <button @click="$wire.set('showConfirmModal', true)" class="ds-btn ds-btn--accent ds-btn--lg" style="flex:1;">Lanjut ke Pembayaran</button>
        <button @click="$wire.set('showCancelModal', true)" class="ds-btn ds-btn--ghost">Batalkan</button>
    </div>

    {{-- ════════ MODAL 1 ════════ --}}
    <x-modal wire:model="showConfirmModal">
        <h2 class="font-display text-2xl font-bold uppercase tracking-[-0.01em] text-ink mb-2">Yakin dengan data ini?</h2>
        ...
        <div class="border border-border-light rounded-xl p-4 mb-4 space-y-2">
            <div><p class="font-body text-[11px] font-semibold text-muted uppercase tracking-[0.06em]">Nama</p><p class="font-body text-sm font-semibold text-ink">{{ auth()->user()->name }}</p></div>
            <div><p class="font-body text-[11px] font-semibold text-muted uppercase tracking-[0.06em]">Email</p><p class="font-body text-[13px] text-ink">{{ auth()->user()->email }}</p></div>
            <div><p class="font-body text-[11px] font-semibold text-muted uppercase tracking-[0.06em]">Tiket</p><p class="inline-block font-display font-bold text-base text-ink bg-accent px-2.5 py-1 rounded-md mt-0.5">{{ $order->qty }} tiket · {{ $order->section->name ?? '—' }}</p></div>
        </div>
        <hr class="border-border-light mb-5">
        <div class="flex gap-3">
            <button wire:click="proceedToPayment" @click="$wire.set('showConfirmModal', false)" class="flex-1 bg-ink text-white font-display font-bold uppercase text-base rounded-pill py-3">Ya, Lanjutkan</button>
    ...
    <x-modal wire:model="showCancelModal">
        <h2 class="font-display text-2xl font-bold uppercase tracking-[-0.01em] text-ink mb-3">Batalkan pemesanan?</h2>
        ...
        <div class="bg-cream border border-border-light rounded-lg p-3 mb-4">
            ...
            <p class="font-display font-bold text-base text-ink mt-0.5">{{ $order->qty }} tiket · {{ $order->section->name ?? '—' }}</p>
        </div>
        <hr class="border-border-light mb-5">
        <div class="flex gap-3">
            <button wire:click="cancelOrder" class="flex-1 bg-[#F55C5C] text-white font-display font-bold uppercase text-base rounded-pill py-3">Ya, Batalkan</button>
